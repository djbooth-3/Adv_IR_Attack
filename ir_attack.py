...
class AdversarialYOLO(YOLO):
    ...
    def attackV9(self, img: List[torch.Tensor], x1: int, y1: int, x2: int, y2: int, area: float):
        ...
        for i in range(250):
            ...
            overlap = np.where(bB_area > 0, i_area / bB_area, 0.0)

            # Apply mask
            mask = conf_mask & (overlap > 0.40) & (iou_scores > 0.40)
...
def main():
    ...
    img_dir = Path("/data/adversarial/ir_images/Seq_1/images")
    ...
    img_label_dir = Path("/data/adversarial/ir_images/Seq_1/Seq1-IR.txt")
